...
export const convertImageListToTableDataType = async (
  item: ProcessedImage,
  index: number,
) => {
  try {
...
export const convertImageListToTableDataTypeSkipCar = async (
  item: ProcessedImage,
  index: number,
) => {
  try {
    ...
    // Use plateCropImage directly if available, otherwise crop from carImage
    let plateImageSrc = "";
    if (item.plateCropImage) {
      const plateImage = await base64ToImage(item.plateCropImage);
      plateImageSrc = plateImage.src;
    } else {
      const plateImage = await cropImage(carImage, item.plateBbox);
      plateImageSrc = plateImage.src;
    }
