...
class ImageProcessor:
  ...
  def read_from_plate_crop(self, plate_crop_image):
    ...
    if plate_crop.size == 0:
      logger.warning("Empty plate crop provided")
    ...
    msg = "OK" if (plate_id or province) else "Plate crop received but OCR/Province failed"

    return self._result_format(
      car_bbox=None,  # Not available when processing plate crop directly
      plate_bbox=None,  # Not available when processing plate crop directly
      text_bbox_list=None,
      plate_id=plate_id,
      province=province,
      full_plate=full_plate,
