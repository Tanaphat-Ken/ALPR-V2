...
def main():
    ...
    # Set device with GPU detection
    if args.device:
        device = torch.device(args.device)
    else:
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    
    # Display GPU info for Google Colab
    ...
    if device.type == "cuda":
        ...
        # Auto-adjust batch size for Google Colab
        if args.batch_size == 64:  # If using default
            if "T4" in gpu_name or "Tesla" in gpu_name:
                args.batch_size = 128
                logger.info(f"  ✓ Auto-adjusted batch size to {args.batch_size} for {gpu_name}")
            elif gpu_memory < 12:
                args.batch_size = 64
                logger.info(f"  ✓ Using batch size {args.batch_size} for limited GPU memory")
    else:
        logger.info("  Running on CPU (will be slow)")
        if args.batch_size > 32:
            args.batch_size = 32
            logger.info(f"  ✓ Reduced batch size to {args.batch_size} for CPU")
    ...
    # Create dataloaders optimized for Google Colab
    # Colab has limited CPU cores, use num_workers=2 for best performance
    num_workers = 2 if args.num_workers == 0 else args.num_workers
    ...
    for epoch in range(start_epoch, args.num_train_epochs):
        ...
        remaining_epochs = args.num_train_epochs - (epoch + 1)
        eta_seconds = avg_epoch_time * remaining_epochs
        eta_minutes = eta_seconds / 60
        eta_hours = eta_minutes / 60
        
        if eta_hours >= 1:
            logger.info(f"⏱️  Epoch time: {epoch_time/60:.1f} min | ETA: {eta_hours:.1f} hours")
        else:
            logger.info(f"⏱️  Epoch time: {epoch_time/60:.1f} min | ETA: {eta_minutes:.0f} minutes")
